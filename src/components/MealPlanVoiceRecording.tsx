...
import { MealPlanVoiceRecordingEditForm } from './MealPlanVoiceRecordingEditForm';

interface MealPlanVoiceRecordingProps {
  isOpen: boolean;
  onClose: () => void;
  onAnalysisComplete: (mealPlan: Omit<MealPlan, 'id' | 'userId'>) => void;
  userId: string;
}

export const MealPlanVoiceRecording = ({ isOpen, onClose, onAnalysisComplete, userId }: MealPlanVoiceRecordingProps) => {
  ...
  const startRecording = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: 'Speak clearly about the meal you want to plan.',
      });
    } catch (error) {
  ...
  const analyzeRecording = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('analyze-meal-plan-voice', {
        body: {
          audio: base64Audio,
          userId: userId,
        },
      ...
      if (!data || typeof data !== 'object' || !data.meal) {
        throw new Error('Invalid response from voice analysis service');
      }

      console.log('Meal plan voice analysis result:', data);
      ...
      toast({
        ...
        description: `Detected meal: "${data.meal.name}". Review and edit the details before adding.`,
      });
  ...
  const handleEditFormSubmit = (mealPlan: Omit<MealPlan, 'id' | 'userId'>) => {
    onAnalysisComplete(mealPlan);
    handleReset();
  };
    ...
    <>
      <Dialog open={isOpen && !showEditForm} onOpenChange={onClose}>
        <DialogContent className="max-w-md glass-card">
          <DialogHeader>
            <DialogTitle>Voice Recording - Meal Plan</DialogTitle>
          </DialogHeader>
          
          <div className="space-y-4">
            <div className="text-sm text-muted-foreground">
              Record yourself describing a meal you want to plan. 
              For example: "I want to make spaghetti carbonara for dinner tomorrow at 7 PM. I'll need pasta, eggs, bacon, and parmesan cheese."
            </div>
      ...
      {showEditForm && analysisResult && (
        <MealPlanVoiceRecordingEditForm
          isOpen={showEditForm}
