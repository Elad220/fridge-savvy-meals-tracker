...
export const MealPlanning = ({ mealPlans, onRemoveMealPlan, onEditMealPlan }: MealPlanningProps) => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-red-50 p-4 rounded-lg shadow-sm">
          ...
          <div className="text-sm text-red-600">Overdue Meals</div>
        </div>
      </div>
      ...
      {sortedMealPlans.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {sortedMealPlans.map((meal) => (
            <div key={meal.id} className={`bg-white border-2 rounded-lg p-4 hover:shadow-md transition-all ${isOverdue(meal) ? 'border-red-500' : 'border-gray-200'}`}>
              <div className="flex justify-between items-start mb-3">
                <h3 className="font-semibold text-gray-900 text-lg">{meal.name}</h3>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${isOverdue(meal) ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>
              ...
              <div className="space-y-2 mb-4">
                ...
                {meal.destinationTime && (
                  <div className={`flex items-center text-sm ${isOverdue(meal) ? 'text-red-600' : 'text-gray-600'}`}>
